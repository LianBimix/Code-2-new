...
(function (Fudge) {
    ...
    var f = FudgeCore;
    Fudge.mesh = new f.MeshSphere("Sphere");
    Fudge.material = new f.Material("Material", f.ShaderLit);
    let canvas;
    const sun = new Fudge.Body("THe sun is a deadly laser", 2, 0, 100);
    const mercury = new Fudge.Body("MAMAAAAAAAAAAAAA", 0.3, 1.2, 50);
    sun.addChild(mercury);
    const venus = new Fudge.Body("Beauty", 0.8, 3, 7);
    sun.addChild(venus);
    //const earth: Body = new Body("Öarph", 3);
    //sun.addChild(earth);
    //const moon: Body = new Body("Luna", 3.5);
    //earth.addChild(moon);
    const mars = new Fudge.Body("Caramel", 0.5, 4, 9.5);
    sun.addChild(mars);
    const phobos = new Fudge.Body("Phobos", 0.3, 0.5, 10);
    const deimos = new Fudge.Body("Deimos", 0.4, 1, 20);
    ...
    function hndLoad() {
        ...
        f.Loop.start();
        f.Loop.addEventListener("loopFrame" /* f.EVENT.LOOP_FRAME */, update); //time-based animation, wird versuchen auf 60fps  zu kommen
    }
    function update() {
        sun.update();
        mercury.update();
